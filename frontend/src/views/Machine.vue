  ...
  <div class="machine-page" :class="{ 'machine-page--kiosk': kioskMode }">
    ...
    <div class="machine-grid">
      <!-- Send wizard — surfaces when the operator hits Send from
           ...
           connection, pick MEM-vs-DNC method, then Send. Hides as
           soon as the stream is in flight or the operator cancels. -->
      <SendWizard
        v-if="showSendWizard"
...
const onWizardCancel = () => {
  ...
  // Strip the ?send query so a refresh doesn't re-open the wizard.
  router.replace({ query: { ...route.query, send: undefined } });
};
const onWizardStarted = () => {
  wizardDismissed.value = true;
  router.replace({ query: { ...route.query, send: undefined } });
  // The cncStore WS will pick up the running status frame.
};
